...
public class OpenUpVipServiceActivity extends BaseServiceActivity {
        ...
        switch (view.getId()) {
            case R.id.imageView_back:
                if (getIntent().getBooleanExtra("center",false)){
                    intent = new Intent(this, VipCenterServiceActivity.class);
                }else {
                    intent = new Intent(this, VipDetailServiceActivity.class);
                }
                ...
                HttpUtils.doPost(Urls.HOST_BUYVIP, mapParams, new Callback() {
                    ...
                    public void onFailure(Call call, IOException e) {
                        runOnUiThread(new Runnable() {
                            @Override
                            public void run() {
                                AppUtils.showToast(OpenUpVipServiceActivity.this, "购买失败");
                            }
                        });
                    }
                    ...
                    public void onResponse(Call call, Response response) throws IOException {
                        try {
                            runOnUiThread(new Runnable() {
                                @Override
                                public void run() {
                                    AppUtils.showToast(OpenUpVipServiceActivity.this, "购买成功");
                                    loadUser();
                                    finish();
                                }
                            });
