...
public class ExpertBookServiceActivity extends BaseServiceActivity implements BookExpertView {
    ...
    private BookExpertAdapter.OnItemClickLitener mOnClickListenner = new BookExpertAdapter.OnItemClickLitener() {
        ...
        public void onItemPlayClick(View view, int position) {
            Intent intent = new Intent(getBaseContext(), ExpertBookDetailServiceActivity.class);
        ...
        public void onPlayClick(View view, int position) {
            ...
            if (NetUtil.isNetConnect(ExpertBookServiceActivity.this)) {
        ...
        public void onGetClick(View view, int position) {
            ...
            if (tv_stage.getText().equals("播放")) {//用户没有刷新没有加载时临时调用（刷新与加载会重新与书架匹配）
                ...
                Intent intent = new Intent(ExpertBookServiceActivity.this, BuyBookServiceActivity.class);
                EventBus.getDefault().postSticky(bookExpertAdapter.getItem(position));
                EventBus.getDefault().removeStickyEvent(MyBookBean.class);
                ...
                if (ifCache) {//读取缓存数据
                    ...
                    if (newString != null) {
                        ...
                        runOnUiThread(new Runnable() {
                            @Override
                            public void run() {
                                AudioPlayer.get().init(ExpertBookServiceActivity.this);
                ...
                HttpUtils.doGet(Urls.HOST_BOOKDETAIL + "/" + bookId, new Callback() {
                    ...
                    public void onFailure(Call call, IOException e) {
                        runOnUiThread(new Runnable() {
                            @Override
                            public void run() {
                                AppUtils.showToast(ExpertBookServiceActivity.this, "播放失败");
                            }
                        });
                    }
